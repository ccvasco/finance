...
const Views = (() => {
  ...
  const COL_DESC = {
    ...
    revenue: "Total sales (Yahoo's trailing-12-month figure) — the base the margin columns are computed against. Yahoo's TTM window and revenue base are its own, so this can differ from the fiscal-year statement revenue shown in the deep-dive charts.",
    operating_income: "Revenue × Op Margin (both Yahoo trailing-12-month figures, so the two columns reconcile with this one). Profit after running the business, before interest & tax — can differ, even in sign, from the fiscal-year statement Operating Income.",
    ocf: "Operating Cash Flow — cash generated by core operations, before investing or financing activity (fiscal-year statement figure; falls back to Yahoo's TTM figure when the statement value is missing). FCF = this + Capex.",
    capex: "Capital Expenditure — cash spent on property, plant & equipment, reported as a negative (a cash outflow) matching the statement. FCF = Op Cash Flow + this figure. Derived from FCF − OCF when the statement doesn't report it directly.",
    ...
    // REIT columns — the deep view's REIT panel as sortable/exportable columns.
    ffo: "Funds From Operations = Net Income + Depreciation & Amortization − property-sale gains + impairments — the REIT-standard earnings measure (approximates NAREIT FFO; treat as directional). Property depreciation is a large non-cash charge buildings usually don't actually lose, so net income understates a REIT's real earning power; FFO adds it back. Equity REITs only — blank for every other row, including mortgage REITs. See REITs.md.",
    ffo_ps: "FFO ÷ shares outstanding — the per-share earning power the dividend is paid from (the REIT analogue of EPS). Equity REITs only.",
    p_ffo: "Price ÷ FFO per share — the REIT equivalent of the P/E ratio, and the standard REIT valuation multiple. Real-world equity REITs commonly trade around 10–20× FFO. Equity REITs only.",
    ffo_payout: "Dividends ÷ FFO. REITs must distribute ~90% of taxable income by law, so this runs higher than a normal payout ratio: ≤80% is a comfortable cushion, ≤100% sustainable, >100% means paying out more than FFO generates. Equity REITs only.",
    ffo_coverage: "FFO ÷ dividends paid — times FFO covers the dividend (the inverse of FFO Payout). Green ≥ 1.25×, yellow 1.0–1.25×, red < 1.0× (paying out more than FFO generates). Equity REITs only.",
    book_value_ps: "Shareholders' equity ÷ shares outstanding — net asset value per share. The primary metric for a mortgage REIT (the stock effectively tracks book value) and ≈ NAV for a fair-value (IFRS) REIT; populated for every row.",
    bvps_growth: "Annualized trend of book value per share, fitted (least-squares) across all available years so one anomalous year can't set the number — the single most important mortgage-REIT quality signal. A mREIT that holds or grows book value while paying its dividend is compounding; a steadily negative trend means the dividend is paid out of capital. Populated for every row.",
    div_coverage_ni: "Net income ÷ dividends paid — how well earnings cover the payout. Below 1.0× the company is distributing more than it earns, a key warning sign for mortgage REITs (whose net income includes volatile mark-to-market swings — read the trend, not one year). Populated for every row.",
    debt_gbv: "Total debt ÷ gross book value — the balance-sheet leverage gauge property REITs covenant against (gross book value = total assets with accumulated depreciation added back). <45% conservative, 45–55% typical, >60% highly levered. Yahoo rarely exposes the add-back for US-GAAP REITs, so the shown ratio then equals Debt/Assets and runs a little high (conservative); exact for fair-value (IFRS) REITs. Property REITs only — blank for mortgage REITs and every non-REIT.",
    debt_to_assets: "Total debt ÷ total assets — the leverage figure the REIT strategy grades score on (≤45% conservative, ≤60% typical, above that aggressive), in place of Debt/Equity, which depreciation distorts for property businesses. Computed for every row; most meaningful for REITs and other asset-heavy businesses.",
  ...
  const PANEL_TIPS = {
    ...
    "Debt/Assets %": "Total debt ÷ total assets — the leverage figure the REIT strategy grades are scored on (bands: ≤45% conservative, ≤60% typical, above that aggressive). It is the row above without the accumulated-depreciation add-back, so the two coincide for most REITs and the gap between them is exactly that add-back. Grading uses this rather than Debt/GBV because total assets is reported by every REIT, while the add-back is exposed for only about one in seven — scoring on it would let a data-feed quirk separate two otherwise identical REITs. It also replaces Debt/Equity, which is unusable for a property business: depreciation erodes book equity every year, so D/E climbs with portfolio age alone, and it inverts entirely once accumulated depreciation exceeds equity.",
    "Debt/GBV %": "Total debt ÷ gross book value — the balance-sheet leverage gauge property REITs covenant against (gross book value = total assets with accumulated depreciation added back, i.e. the asset base at undepreciated cost). Declarations of trust commonly cap it near 60%: <45% is conservative, 45–55% typical, >60% highly levered. Caveat: Yahoo reports US-GAAP REIT property at net (depreciated) value with no accumulated-depreciation line, so for those the true ratio is a little lower than shown; for a fair-value (IFRS) REIT total assets already sit at fair value, making the figure exact.",
  ...
  const COLS = [
    ...
    { key: "revenue", label: "Revenue", fmt: (v, r) => Fmt.big(v, r.financial_currency || r.currency) },
    { key: "operating_income", label: "Op Income", fmt: (v, r) => Fmt.big(v, r.financial_currency || r.currency) },
    ...
    { key: "debt_to_assets", label: "Debt/Assets", fmt: (v) => Fmt.pct(v, 1) },
    ...
    { key: "ocf", label: "Op Cash Flow", fmt: (v, r) => Fmt.big(v, r.financial_currency || r.currency) },
    { key: "capex", label: "Capex", fmt: (v, r) => Fmt.big(v, r.financial_currency || r.currency) },
    // dividend
    ...
    // REIT — the deep view's REIT panel as sortable columns (and thereby in the
    // Excel export, which mirrors this list). The FFO family is equity-REIT-only
    // and Debt/GBV property-REIT-only — blank everywhere else; BV/Share,
    // BV Trend and Div Cov (NI) are generic and populated for every row.
    { key: "ffo", label: "FFO", fmt: (v, r) => Fmt.big(v, r.financial_currency || r.currency) },
    { key: "ffo_ps", label: "FFO/Share", fmt: (v, r) => Fmt.price(v, r.financial_currency || r.currency) },
    { key: "p_ffo", label: "P/FFO", fmt: (v) => Fmt.num(v) },
    { key: "ffo_payout", label: "FFO Payout", fmt: (v) => Fmt.ratioPct(v) },
    { key: "ffo_coverage", label: "FFO Cov", fmt: (v) => v == null ? null : Fmt.num(v) + "×", cls: gradeClass(1.25, 1.0) },
    { key: "book_value_ps", label: "BV/Share", fmt: (v, r) => Fmt.price(v, r.financial_currency || r.currency) },
    { key: "bvps_growth", label: "BV Trend", fmt: (v) => Fmt.pct(v), signed: true },
    { key: "div_coverage_ni", label: "Div Cov (NI)", fmt: (v) => v == null ? null : Fmt.num(v) + "×" },
    { key: "debt_gbv", label: "Debt/GBV", fmt: (v) => Fmt.pct(v, 1) },
    // risk
